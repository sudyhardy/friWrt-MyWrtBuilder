...
passwall_file_down="$(curl -s ${passwall_api} | grep "browser_download_url" | grep -oE "https.*passwall2_packages_ipk_$ARCH_3.zip" | head -n 1)"
...
if [ "$1" == "openclash" ]; then
    ...
    if [ $? -eq 0 ]; then
        # Extract using the actual downloaded filename
        downloaded_file=$(basename "$passwall_file_down")
        unzip -qq packages/"$downloaded_file" -d packages && rm packages/"$downloaded_file"
    fi
    wget "$passwall_ipk" -nv -P packages
    for pkg in "${passwall_ipk_packages[@]}"; do
        wget "$pkg" -nv -P packages
    done
    wget "${passwall2_file_down}" -nv -P packages
    rm -f files/usr/bin/patchoc.sh
